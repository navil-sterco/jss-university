...
const Mapping = ({ courses, schools, pages, departments }) => {
    const { data, setData, post, processing, errors } = useForm({
        school_ids: courses.schools?.map((s) => s.id) || [],
        page_ids: courses.pages?.map((p) => p.id) || [],
        department_ids: courses.departments?.map((d) => d.id) || [],
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route("course.mapping.attach", courses.id), {
            preserveScroll: true,
        });
    };
        ...
        <div className="container py-4">
            ...
            <div className="card shadow-sm">
                <div className="card-header bg-white">
                    <h5>
                        Map Courses:{" "}
                        <span className="text-grey">{courses.title}</span>
                    </h5>
                </div>
