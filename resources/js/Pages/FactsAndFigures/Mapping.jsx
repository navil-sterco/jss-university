...
const Mapping = ({ factsandfigures, schools, pages, departments, courses }) => {
    const { data, setData, post, processing, errors } = useForm({
        school_ids: factsandfigures.schools?.map((s) => s.id) || [],
        page_ids: factsandfigures.pages?.map((p) => p.id) || [],
        department_ids: factsandfigures.departments?.map((d) => d.id) || [],
        course_ids: factsandfigures.courses?.map((c) => c.id) || [],
        show_on_home: factsandfigures.show_on_home || false,
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route("facts-and-figures.mapping.attach", factsandfigures.id), {
            preserveScroll: true,
        });
    };
            ...
            <div className="card shadow-sm">
                <div className="card-header bg-white">
                    <h5>
                        ...
                        <span className="text-grey">{factsandfigures.title}</span>
                    </h5>
                </div>
                                ...
                                <div className="card">
                                    ...
                                    <div className="card-body">
                                        ...
                                        <div className="form-text">
                                            {data.show_on_home 
                                                ? "This Facts And Figures will be displayed on the homepage"
                                                : "This Facts And Figures will not be displayed on the homepage"
                                            }
                                        </div>
                                        {errors.show_on_home && (
