...
const Mapping = ({ faculties, schools, pages, departments, courses }) => {
    const { data, setData, post, processing, errors } = useForm({
        school_ids: faculties.schools?.map((s) => s.id) || [],
        page_ids: faculties.pages?.map((p) => p.id) || [],
        department_ids: faculties.departments?.map((d) => d.id) || [],
        course_ids: faculties.courses?.map((c) => c.id) || [],
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route("faculty.mapping.attach", faculties.id), {
            preserveScroll: true,
        });
    };
        ...
        <div className="container py-4">
            ...
            <div className="card shadow-sm">
                <div className="card-header bg-white">
                    <h5>
                        Map Faculty:{" "}
                        <span className="text-grey">{faculties.title}</span>
                    </h5>
                </div>
