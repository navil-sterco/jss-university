...
const Mapping = ({ happening, schools, pages, departments, courses }) => {
    const { data, setData, post, processing, errors } = useForm({
        school_ids: happening.schools?.map((s) => s.id) || [],
        page_ids: happening.pages?.map((p) => p.id) || [],
        department_ids: happening.departments?.map((d) => d.id) || [],
        course_ids: happening.courses?.map((c) => c.id) || [],
        show_on_home: happening.show_on_home || false,
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route("happening.mapping.attach", happening.id), {
            preserveScroll: true,
        });
    };
        ...
        <div className="container py-4">
            ...
            <div className="card shadow-sm">
                <div className="card-header bg-white">
                    <h5>
                        Map Happening:{" "}
                        <span className="text-grey">{happening.title}</span>
                    </h5>
                </div>
