...
const Edit = ({ header }) => {
    const { schools, departments, pages, menuItems } = usePage().props;
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route("headers.update", header.id));
    };

    const handleTypeChange = (type) => {
        setData('type', type);
        // Reset reference_id and url when type changes
        if (type !== data.type) {
            setData('reference_id', '');
            setData('url', '');
            // Don't reset title if we're switching to custom
            if (type === 'custom') {
                setData('title', '');
            }
        }
                        ...
                        <div className="row">
                            ...
                            <div className="mb-3 col-md-6">
                                ...
                                >
                                    ...
                                    {menuItems?.filter(item => !item.parent_id && item.id !== header.id).map(item => (
