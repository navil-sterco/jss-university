...
const Mapping = ({ programs, schools, pages, departments }) => {
    const { data, setData, post, processing, errors } = useForm({
        school_ids: programs.schools?.map((s) => s.id) || [],
        page_ids: programs.pages?.map((p) => p.id) || [],
        department_ids: programs.departments?.map((d) => d.id) || [],
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route("program.mapping.attach", programs.id), {
            preserveScroll: true,
        });
    };
        ...
        <div className="container py-4">
            ...
            <div className="card shadow-sm">
                <div className="card-header bg-white">
                    <h5>
                        Map programs:{" "}
                        <span className="text-grey">{programs.title}</span>
                    </h5>
                </div>
