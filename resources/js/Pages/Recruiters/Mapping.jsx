...
const Mapping = ({ recruiters, schools, pages, departments, courses }) => {
    const { data, setData, post, processing, errors } = useForm({
        school_ids: recruiters.schools?.map((s) => s.id) || [],
        page_ids: recruiters.pages?.map((p) => p.id) || [],
        department_ids: recruiters.departments?.map((d) => d.id) || [],
        course_ids: recruiters.courses?.map((c) => c.id) || [],
        show_on_home: recruiters.show_on_home || false,
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route("recruiters.mapping.attach", recruiters.id), {
            preserveScroll: true,
        });
    };
        ...
        <div className="container py-4">
            ...
            <div className="card shadow-sm">
                <div className="card-header bg-white">
                    <h5>
                        Map Recruiter:{" "}
                        <span className="text-grey">{recruiters.title}</span>
                    </h5>
                </div>
                                ...
                                <div className="card">
                                    ...
                                    <div className="card-body">
                                        ...
                                        <div className="form-text">
                                            {data.show_on_home 
                                                ? "This Recruiter will be displayed on the homepage"
                                                : "This Recruiter will not be displayed on the homepage"
                                            }
                                        </div>
                                        {errors.show_on_home && (
